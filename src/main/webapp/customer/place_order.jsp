                ...
                <div class="space-y-4">
                    <div class="bg-white rounded-2xl shadow-lg p-6 sticky top-20">
                        ...
                        <div class="mt-5 space-y-3">
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 mb-1">Expected Delivery</label>
                                <select id="orderUrgency"
                                    class="w-full border-2 border-gray-200 rounded-xl px-3 py-2 text-sm focus:border-purple-400 outline-none bg-white">
                            ...
                            <div class="bg-gray-50 border border-gray-100 p-4 rounded-xl">
                                <p class="text-xs text-gray-500 font-bold uppercase mb-1">Billing & Delivery Details</p>
                                <p class="text-sm font-semibold text-gray-800" id="displayShopName">Loading...</p>
                                <p class="text-xs text-gray-500 mt-1" id="displayShopAddress">Loading...</p>
                                <p class="text-xs text-gray-500 mt-0.5" id="displayShopPhone">Loading...</p>
                                <a href="profile.jsp"
                                    class="text-xs text-purple-600 font-bold mt-2 inline-block hover:underline">Edit
                                    Details</a>
                            </div>
                        </div>
        ...
        <script>
            ...
            function updateSummary() {
                ...
                for (const [k, qty] of Object.entries(cart)) {
                    if (qty > 0) {
                        html += '<div class="flex justify-between"><span>' + labels[k] + ' × ' + qty + '</span><span class="font-semibold">&#x20B9;' + (prices[k] * qty).toLocaleString('en-IN') + '</span></div>';
                        total += prices[k] * qty;
                    }
                }
            ...
            function placeOrder() {
                ...
                const shop = profile.shop || 'Ramesh General Store';
                const addr = profile.addr || 'Village Khari, Dist. Anand';
                const phone = profile.phone || '+91 94001 11111';
                ...
                orders.unshift({ id: orderId, shop: shop, addr: addr, phone: phone, items: itemsStr, amount: amt, status: 'Pending', urgency: urgency, date: new Date().toLocaleString('en-IN') });
            ...
            document.addEventListener('DOMContentLoaded', () => {
                const profile = JSON.parse(localStorage.getItem('cs_profile_cust1') || '{}');
                document.getElementById('displayShopName').innerText = profile.shop || 'Ramesh General Store';
                document.getElementById('displayShopAddress').innerText = profile.addr || 'Village Khari, Dist. Anand';
                document.getElementById('displayShopPhone').innerText = profile.phone || '+91 94001 11111';
            });
        </script>
    </body>
